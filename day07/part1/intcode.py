...
def perform_input(program, address, inpt):
    if inpt is None:
        program[program[address+1]] = int(input("input > "))
    else:
        program[program[address+1]] = inpt
    return program, (address + 2)

def perform_output(program, address, parameterAccessModes):
    output = get_data(program, address+1, parameterAccessModes[0])
    #print(output)
    return program, (address + 2), output
...
def run(program, inputs):
    ...
    while opcode != 99:
        if opcode == 1:
            ...
            if inputs is None:
                program, instruction_pointer = perform_input(program, instruction_pointer, None)
            else:
                program, instruction_pointer = perform_input(program, instruction_pointer, inputs[currentInput])
                currentInput += 1
        elif opcode == 4:
            program, instruction_pointer, currentOutput = perform_output(program, instruction_pointer, parameterAccessModes)
        elif opcode == 5:
            ...
            return program, opcode, currentOutput
        opcode, parameterAccessModes = parse_instruction(program[instruction_pointer])
