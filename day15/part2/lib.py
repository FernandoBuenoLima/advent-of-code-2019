...
class DroidControl:
    ...
    def transportOxygen(this, p):
        adjacentPositions = []
        adjacentPositions.append([p[0], p[1]-1])
        adjacentPositions.append([p[0]+1, p[1]])
        adjacentPositions.append([p[0], p[1]+1])
        adjacentPositions.append([p[0]-1, p[1]])
        
        oxygenPositions = []
        
        for ap in adjacentPositions:
            if this.getPositionContent(ap) == EMPTY:
                oxygenPositions.append(ap)
                this.map[ap[0]][ap[1]] = OXYGEN_SYSTEM
                
        return oxygenPositions
    
    def mapHasEmptySpaces(this):
    ...
    def getOxygenSystemPosition(this):
        for y in range(len(this.map)):
            for x in range(len(this.map[0])):
                if this.getPositionContent([x,y]) == OXYGEN_SYSTEM:
                    return [x,y]
    
    def fillMapWithOxygen(this):
        minutes = 0
        oxygenPositions = [this.getOxygenSystemPosition()]
        while this.mapHasEmptySpaces():
            minutes += 1
            nOxygenPositions = []
            for p in oxygenPositions:
                nOxygenPositions.extend(this.transportOxygen(p))
            oxygenPositions = nOxygenPositions
        return minutes
